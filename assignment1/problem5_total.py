...
d0 = 1.0 
eta = 3               # (3.3 really good for 10 and beyond) (2.6 good for 7-9) (3 is an ok middle ground)
kref_db = -2 
sigma_db = 3.8       # log-normal fading std dev in db (1.5 seems pretty good)
...
total_powers = np.zeros(len(float_rec_coords) * len(float_trans_coords))
total_distances = np.zeros(len(float_rec_coords) * len(float_trans_coords))
total_index = 0
...
for k in range(7, 19):
    ...
    for i in range(len(rec_data)):
        total_distances[total_index] = plotted_rec_distances[i]
        total_powers[total_index] = rec_data[i]
        total_index += 1
...
for i in range(len(total_powers)):
    if (total_powers[i] == 0.0):
        total_indices_to_remove.append(i)
        print("Detected")

if (len(total_indices_to_remove) > 0):
    total_distances = np.delete(total_distances, total_indices_to_remove)
    total_powers = np.delete(total_powers, total_indices_to_remove)
...
plt.scatter(total_distances, (-1 * total_powers), marker='o', label=f"Experiment data")
plt.xlabel('Distance (m)')
plt.ylabel('Avg Received Power (dB)')
plt.title('Received Power (dB) vs Distance')
plt.grid(True)
